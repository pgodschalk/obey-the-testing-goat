...
def given_i_am_logged_in(context):
    session_key = create_pre_authenticated_session(email="edith@example.org")
...
def wait_for_list_item(context, item_text):
    context.test.assertIn(
        item_text, context.browser.find_element_by_css_selector("#id_list_table").text
    )
...
def on_list_page(context, first_item_text):
    ...
    expected_row_test = "1: " + first_item_text
    context.test.assertEqual(first_row.text, expected_row_test)
